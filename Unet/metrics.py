...
if __name__ == "__main__":
    with torch.no_grad():
        ...
        for images, labels in tqdm(valid_loader):
            ...
            image = transforms.ToPILImage()(images.squeeze())
            output = model(image)
            ...
            segmentation_mask = torch.argmax(segmentation, dim=0)
            segmentation_mask_tensor = segmentation_mask.cpu()
        ...
        overall_mean_accuracy = sum(class_metrics[i]['accuracy'] for i in range(1,15)) / (14 * len(valid_loader))
        overall_mean_precision = sum(class_metrics[i]['precision'] for i in range(1,15)) / (14 * len(valid_loader))
        overall_mean_recall = sum(class_metrics[i]['recall'] for i in range(1,15)) / (14 * len(valid_loader))
        overall_mean_map50 = sum(class_metrics[i]['map50'] for i in range(1,15)) / (14 * len(valid_loader))
